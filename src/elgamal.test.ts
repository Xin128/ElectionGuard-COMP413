...
describe("TestElgamal", () => {
    test('test_simple_elgamal_encryption_decryption', () => {
        ...
        const keypair:ElGamalKeyPair|undefined = elgamal_keypair_from_secret(secret_key);
        const public_key:number = keypair.public_key;
        expect(public_key.to_int()).toBeLessThan(P);
        expect(Math.pow(public_key.to_int(), nonce.to_int())).toBe(P);

        
        const elem:ElementModP = g_pow_p(ZERO_MOD_Q);
        expect(elem.equals(ONE_MOD_P)).toBe(true); // g^0 == 1

        const ciphertext:ElGamalCiphertext|undefined = elgamal_encrypt(0, nonce, keypair.public_key);
        expect(G).toBe(ciphertext.pad.to_int());
        expect(Math.pow(ciphertext.pad.to_int(), secret_key.to_int())).toBe(P));
