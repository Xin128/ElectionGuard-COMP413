...
import { bnToHex, getRandomIntExclusive, } from "./groupUtils_sjcl";
...
const Q = new sjcl.bn(BigInt('115792089237316195423570985008687907853269984665640564039457584007913129639747').toString(16));
const P = new sjcl.bn(BigInt('').toString(16));
// // ((P - 1) * pow(Q, -1, P)) % P
// // rewrite pow(a,-b,c) as pow((a^-1) mod c, b, c)
const R = new sjcl.bn(BigInt('').toString(16));
// const R: bigint = ((P - BigInt(1)) * bigintModArith.modPow(Q, -1, P)) % P;
// // const R: bigint = ((P - 1n) * (powmod(Q, P) % P)) % P;
const G  = new sjcl.bn(BigInt('').toString(16));
const Q_MINUS_ONE: sjcl.BigNumber = Q.sub(new sjcl.bn(1));

// Large const in electionguard python

const mod: (a: sjcl.BigNumber, b: sjcl.BigNumber) => sjcl.BigNumber = (a, b) => {
    // return ((a % b) + b) % b
...
class ElementModQ {
    ...
    @Type(() => BigInt)
    public elem: bigint;
    private core: sjcl.BigNumber;

    constructor(elem: bigint | number | sjcl.BigNumber) {
      if (typeof elem === "bigint") {
        this.elem = elem;
        this.core = new sjcl.bn(elem.toString(16));
      } else if (typeof elem === "number") {
        this.elem = BigInt(elem);
        this.core = new sjcl.bn(elem);
      } else {
        //sjcl.bignumber
        this.core = elem.copy();
        this.elem = BigInt(elem.toString());
      }
    }

    public get_core(): sjcl.BigNumber {
      return this.core;
    }

    public set_core(input: sjcl.BigNumber): void{
      this.elem = BigInt(input.toString());
      this.core = input.copy();
    }
    ...
    public is_in_bounds(): boolean {
       ...
       return this.core.greaterEquals(0) && !(this.core.greaterEquals(Q));
    }
    ...
    public is_in_bounds_no_zero(): boolean {
        // return 0 < this.elem && this.elem < Q;
        return (this.core.greaterEquals(0) && !this.core.equals(0)) && !(this.core.greaterEquals(Q));
    }
...
class ElementModP {
    ...
    constructor(elem: bigint | sjcl.BigNumber) {
      if (typeof elem === "bigint") {
        this.elem = elem;
        this.core = new sjcl.bn(elem.toString(16));
      } else {
    ...
    public is_in_bounds(): boolean {
        ...
        return (this.core.greaterEquals(0)) && !(this.core.greaterEquals(P));
    }

    public is_in_bounds_no_zero(): boolean {
        // return 0 < this.elem && this.elem < P;
        return (this.core.greaterEquals(0) && !this.core.equals(0)) && !this.core.greaterEquals(P);
    }

    public is_valid_residue(): boolean {
        const residue: boolean = pow_p(this, new ElementModQ(BigInt(Q.toString()))).equals(ONE_MOD_P);
...
const G_MOD_P: ElementModP = new ElementModP(BigInt(G.toString()));
...
const hex_to_q: (input: string) => ElementModQ | null = (input) => {
    const i = new sjcl.bn(input);
    // if (0 <= i && i < Q) {
    if (i.greaterEquals(0) && !i.greaterEquals(Q)) {
        return new ElementModQ(BigInt(input));
    } else {
        return null;
    }
}
...
// Returns `None` if the bigint is out of the allowed
// [0,Q) range.
const int_to_q: (input: bigint) => ElementModQ | null = (input) => {
    const i = new sjcl.bn(input.toString(16));
    // if (0 <= i && i < Q) {
    if (i.greaterEquals(0) && !i.greaterEquals(Q)) {
        return new ElementModQ(input);
    } else {
        return null;
    }
}
...
const int_to_q_unchecked: (i: bigint) => ElementModQ = (i) => {
    return new ElementModQ(i);
}

// Given an integer, returns an ElementModP.
// Returns `None` if the bigint is out of the allowed
// [0,P) range.
const int_to_p: (input: bigint) => ElementModP | null = (input) => {
    const i = new sjcl.bn(input.toString(16));
    // if (0 <= i && i < P) {
    if (i.greaterEquals(0) && !i.greaterEquals(P)) {
        return new ElementModP(input);
    } else {
        return null;
    }
}
...
//     you're absolutely, positively, certain the input is in-bounds.
const int_to_p_unchecked: (i: bigint) => ElementModP = (i) => {
    return new ElementModP(i);
}
...
const a_minus_b_q: (a: ElementModQorInt, b: ElementModQorInt) => ElementModQ = (a, b) => {
    ...
    return new ElementModQ(mod(a.get_core().sub(b.get_core()),Q));
}
...
const div_p: (a: ElementModPOrQorInt, b: ElementModPOrQorInt) => ElementModP = (a, b) => {
    ...
    // const inverse = b.elem.modPow(-1, P);
    // // const inverse = powmod(b.elem, P);

    // const inverse = b.elem.modPow(-1, P);
    const inverse = b.get_core().inverseMod(P);
    // return mult_p(a, int_to_p_unchecked(inverse));
    return mult_p(a, new ElementModP(inverse));
}
...
const div_q: (a: ElementModPOrQorInt, b: ElementModPOrQorInt) => ElementModQ = (a, b) => {
    ...
    const inverse = b.get_core().inverseMod(Q);
  // const inverse = powmod(b.elem, Q);
  //   return mult_q(a, int_to_q_unchecked(inverse));
    return mult_q(a, new ElementModQ(inverse));
}
...
const negate_q: (a: ElementModQorInt) => ElementModQ = (a) => {
    ...
    return new ElementModQ(Q.sub(a.get_core()));
}
...
const a_plus_bc_q: (a: ElementModQorInt, b: ElementModQorInt, c: ElementModQorInt) => ElementModQ = (a, b, c) => {
    ...
    return new ElementModQ(a.get_core().add(b.get_core().mul(c.get_core())).mod(Q));
}
...
const mult_inv_p: (e: ElementModPOrQorInt) => ElementModP = (e) => {
    ...
    return new ElementModP(e.get_core().inverseMod(P));
}
...
const pow_p: (b: ElementModPOrQorInt, e: ElementModPOrQorInt) => ElementModP = (b, e) => {
    ...
    return new ElementModP(b.get_core().powermod(e.get_core(), P));
}
...
const pow_q: (b: ElementModQorInt, e: ElementModQorInt) => ElementModQ = (b, e) => {
    ...
    return new ElementModQ(b.get_core().powermod(e.get_core(), Q));
}
...
const mult_p: (...elems: ElementModPOrQorInt[]) => ElementModP = (...elems) => {
    let product = new sjcl.bn(1);
    elems.forEach((x) => {
        if (typeof x === 'bigint') {
            x = int_to_p_unchecked(x);
        }
        // product = (product * x.elem) % P;
        product = product.mulmod(x.get_core(), P);
    })
...
const mult_q: (...elems: ElementModPOrQorInt[]) => ElementModQ = (...elems) => {
    ...
    elems.forEach((x) => {
        ...
        product = product.mulmod(x.get_core(), Q);
    })
...
const rand_range_q: (start: ElementModQorInt) => ElementModQ = (start) => {
    ...
    let random = new sjcl.bn(0);
    // while (random < start) {
    const start_bn = new sjcl.bn(start.toString());
    while (!random.greaterEquals(start_bn)) {
...
const eq_elems: (a: ElementModPOrQ, b: ElementModPOrQ) => boolean = (a, b) => {
    ...
    return a.get_core().equals(b.get_core());
}
